...
static void weather_update_icon(RailsState *state, int icon_code) {
    GDrawCommandImage *new_icon = NULL;

    switch (icon_code) {
        case WEATHER_ICON_CLEAR_DAY:
            new_icon = state->weather_clear_day_icon;
            break;
        case WEATHER_ICON_CLEAR_NIGHT:
            new_icon = state->weather_clear_night_icon;
            break;
        case WEATHER_ICON_CLOUDY_DAY:
            new_icon = state->weather_cloudy_icon;
            break;
        case WEATHER_ICON_HEAVY_RAIN:
            new_icon = state->weather_rain_heavy_icon;
            break;
        case WEATHER_ICON_HEAVY_SNOW:
            new_icon = state->weather_snow_heavy_icon;
            break;
        case WEATHER_ICON_LIGHT_RAIN:
            new_icon = state->weather_rain_light_icon;
            break;
        case WEATHER_ICON_LIGHT_SNOW:
            new_icon = state->weather_snow_light_icon;
            break;
        case WEATHER_ICON_PARTLY_CLOUDY_NIGHT:
            new_icon = state->weather_partly_cloudy_night_icon;
            break;
        case WEATHER_ICON_PARTLY_CLOUDY:
            new_icon = state->weather_partly_cloudy_icon;
            break;
        case WEATHER_ICON_RAINING_AND_SNOWING:
            new_icon = state->weather_rain_and_snow_icon;
            break;
        case WEATHER_ICON_THUNDERSTORM:
            new_icon = state->weather_thunderstorm_icon;
            break;
        default:
            new_icon = state->weather_generic_icon;
            break;
    }

    // Only mark field icon layers dirty if the icon actually changed
    if (state->current_weather_icon != new_icon) {
        state->current_weather_icon = new_icon;
